\message{: version 25apr05, but with voffset.5truein, dfont bf, and toc simplified}
...
\def\RRt{{\titlefont I}\kern-.2em{\titlefont R}}
...
% the following sizes were used on the first two papers
\magnification\magstep0
\hsize12.1truecm\vsize18.6truecm
\hoffset.8truein
\voffset.5truein
% the following sizes are used subsequently
%\magnification\magstephalf
%\hsize6.5truein\vsize8.6truein
%\voffset.5truein

% Do we need to set the size so that it looks ok in A4 and Letterhead?
...
\ifx\next\empty\else\diamondleaders\fi\hfil\hbox to 2em{\hss##2}}}}
...
% this is the tocline used originally
%\def\tocline#1#2#3{\nexttoc{\line{\hskip\parindent\noexpand\noexpand\noexpand\rm\hskip#2truecm #1\hfill#3\hskip\parindent}}}
% this is the tocline used subsequently
...
\leftline{Surveys in Approximation Theory
  \hfill {\ninerm \the\pageno}}
\leftline{Volume 1, 2005.
pp.~\the\pageno--\the\count102.}
\leftline{Copyright \copyright\ 2005 Surveys in Approximation Theory.}
...
\def\sect#1{\startsect\edef\showsectno{\the\sectionno}\let\tocindent\tocone%
       \soneormore#1||||:\relax\medskip\noindent\ignorespaces}
  %% DEFINE SECTION HEADING
  %% if overlong, use || to introduce line breaks

\def\soneormore#1||#2||#3:{%
   \leftline{\bf\showsectno\hskip2em #1}
   \def\next{#2}%
   \ifx\next\empty\puttocline{\showsectno\ \ #1}{\folio}%
...
\def\subsect#1{\formal{#1.}\let\tocindent\toctwo\puttocline{#1}{\folio}}
